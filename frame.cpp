...
Frame::Frame(quint8 cmd, quint8 data, QObject *parent) : QObject(parent)
{
    ...
    m_buffer.insert(INDEX_DATA_LENGTH, 1);
    m_buffer.insert(INDEX_FIRST_DATA_BYTE, data);
...
Frame::Frame(quint8 cmd, quint16 data, QObject *parent) : QObject(parent)
{
    ...
    m_buffer.insert(INDEX_FIRST_DATA_BYTE, quint8((data & 0xFF00) >> 8));
    m_buffer.insert((INDEX_FIRST_DATA_BYTE + 1) , quint8(data & 0x00FF));
    m_buffer += CalculateChecksum();
}
